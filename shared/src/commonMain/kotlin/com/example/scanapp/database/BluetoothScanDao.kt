...
class BluetoothScanDao(private val database: ScanAppDatabase) {
    ...
    suspend fun getRecordsPaginated(limit: Int, offset: Int): List<BluetoothScanRecord> = withContext(Dispatchers.Default) {
        database.scanAppQueries.selectBluetoothRecordsPaginated(limit = limit.toLong(), offset = offset.toLong()).executeAsList().map {
            BluetoothScanRecord(
                id = it.id,
                name = it.name,
                address = it.address,
    ...
    suspend fun getCount(): Long = withContext(Dispatchers.Default) {
        database.scanAppQueries.countBluetoothRecords().executeAsOne()
    }

    suspend fun getRecordsByDeviceType(deviceType: String): List<BluetoothScanRecord> = withContext(Dispatchers.Default) {
        database.scanAppQueries.selectBluetoothRecordsByType(deviceType).executeAsList().map {
            BluetoothScanRecord(
                id = it.id,
                name = it.name,
                address = it.address,
